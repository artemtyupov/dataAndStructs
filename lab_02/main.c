...
void search(void) {
    ...
    if (input_string("\nInput color to search:\n", str, STR_LEN) == 0) {
        ...
        for (int i = 0; i < table.n; i++) {
            char *istr;
            istr = strstr(ptr_cur->color, str);
            if ((strcmp(ptr_cur->color, str) == 0)||((istr != NULL) & (strlen(ptr_cur->color) - 1 == (strlen(str))) &
                    ( ptr_cur->used == 1) & (ptr_cur->united.used_car.mileage < 10000) &
                    (ptr_cur->united.used_car.repairs_count == 0))){
                printf("[ Record %d ]\n", (i + 1));
                print_car(ptr_cur);
                printf("--------------------------------------------\n");
                found++;
