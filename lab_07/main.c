            ...
            else
            {
                while(choice)
                {
                    if(err == OK)
                    {
                        ...
                        printf("%s", "11 - Print tree in file for visual.\n");
                    ...
                    case 9:
                        ...
                        if (count > 3)
                        {
                            tree *new_tr = (tree*)malloc(sizeof(tree));
                            init_tree(new_tr);
                            Hash_row **new_open_hashtable;
                            node *temp_new = (node*)malloc(sizeof(node));
                            f = fopen("Input.txt", "r");
                            n_open = n_open * 2;
                            new_open_hashtable = init_hash(n_open);
                            while(fscanf(f, "%s\n", temp_new->keyword) == 1)
                            {
                                fgets(temp_new->HELP, 1000, f);
                                add_open_hash(new_open_hashtable, temp_new, n_open);
                            }
                            fclose(f);
                            open_hashtable = new_open_hashtable;
                        }
                        count = 0;
                        closed_hash_search(closed_hashtable, keyword, n_closed, &count, 0);
                        if (count > 3)
                        {
                            Hash_row **new_closed_hashtable;
                            node *temp_new = (node*)malloc(sizeof(node));
                            f = fopen("Input.txt", "r");
                            n_closed = n_closed * 2;
                            new_closed_hashtable = init_hash(n_closed);
                            while(fscanf(f, "%s\n", temp_new->keyword) == 1)
                            {
                                fgets(temp_new->HELP, 1000, f);
                                err = add_closed_hash(closed_hashtable, temp_new, n_closed);
                            }
                            fclose(f);
                            closed_hashtable = new_closed_hashtable;
                        }
                        break;
                    case 10:
                        printf("%s", "Input a keyword: ");
                        ...
                        break;
                    case 11:
                        f3 = fopen("Graph.txt", "w");
                        fprintf(f3, "digraph G {\n");
                        print_tree_file(tr->root, "", f3, 0);
                        fprintf(f3, "}");
                        break;
